...
WHERE
	(
		(
			(
				...
				AND (
					cun.warnning_deadline = 1
					OR cun.warnning_deadline_m = 1
					OR cun.warnning_deadline_mm = 1
				)
				AND cun.time_limit > 0
			)
	...
	cun.warnning_deadline_mm,
	cun.deadline_mm,
	ai.affairs_info_id AS ai_affairs_info_id,
	ai.split_code AS ai_split_code,
	p.affairs_proposers_id AS p_affairs_proposers_id,
	p.split_code AS p_split_code,
	us.user_id AS us_user_id,
	reg.region_id AS reg_region_id,
	pt.credential_type_id AS pt_credential_type_id,
	pg.gender_id AS pg_gender_id,
	af.affairs_id AS af_affairs_id,
	bm.department_id AS bm_department_id
FROM
	tab_affairs_info AS ai
INNER JOIN (
	SELECT
