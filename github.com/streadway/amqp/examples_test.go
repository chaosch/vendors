...
func ExampleConfig_timeout() {
	...
	conn, err := DialConfig("amqp:///", Config{
...
func ExampleDialTLS() {
	...
	conn, err := DialTLS("amqps://server-name-from-certificate/", cfg)
...
func ExampleChannel_Confirm_bridge() {
	...
	destination, err := Dial("amqp://destination/")
	if err != nil {
	...
	confirms := chd.NotifyPublish(make(chan Confirmation, 1))
	...
	for {
		...
		err = chd.Publish("logs", msg.RoutingKey, false, false, Publishing{
...
func publishAllTheThings(conn *Connection) {
...
func ExampleConnection_NotifyBlocked() {
	...
	blockings := conn.NotifyBlocked(make(chan Blocking))
	go func() {
