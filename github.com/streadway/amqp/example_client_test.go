...
type Session struct {
	...
	notifyConfirm   chan Confirmation
	isReady         bool
}
...
func (session *Session) connect(addr string) (*Connection, error) {
	conn, err := Dial(addr)
...
func (session *Session) handleReInit(conn *Connection) bool {
	for {
		session.isReady = false
...
func (session *Session) init(conn *Connection) error {
	ch, err := conn.Channel()
...
func (session *Session) changeConnection(connection *Connection) {
	session.connection = connection
	session.notifyConnClose = make(chan *Error)
	session.connection.NotifyClose(session.notifyConnClose)
}

// changeChannel takes a new channel to the queue,
// and updates the channel listeners to reflect this.
func (session *Session) changeChannel(channel *Channel) {
	session.channel = channel
	session.notifyChanClose = make(chan *Error)
	session.notifyConfirm = make(chan Confirmation, 1)
...
func (session *Session) Stream() (<-chan Delivery, error) {
	if !session.isReady {
