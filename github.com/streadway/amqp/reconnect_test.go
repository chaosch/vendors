...
func setup(url, queue string) (*Connection, *Channel, error) {
	conn, err := Dial(url)
	if err != nil {
		return nil, nil, err
	}
...
func consume(url, queue string) (*Connection, <-chan Delivery, error) {
...
func ExampleConnection_reconnect() {
	if url := os.Getenv("AMQP_URL"); url != "" {
		...
		for i := 1; i <= 3; i++ {
			...
			if err := pub.Publish("", queue, false, false, Publishing{
