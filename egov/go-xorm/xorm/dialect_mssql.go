...
func (db *mssql) GetTablesSingle(tableName string) ([]*core.Table, error) {
	args := []interface{}{}
	s := `select name from sysobjects where xtype ='U' and name='` + tableName + `'`
	db.LogSQL(s, args)
...
ixcs.is_descending_key as desc,
ixcs.key_ordinal as ordinal
...
`
	...
	for rows.Next() {
		...
		err = rows.Scan(&indexName, &colName, &isUnique, &desc, &ordinal)
		if err != nil {
			return nil, err
		}
...
func (db *mssql) CreateRenameIndexSql(tableName string, oldIndex, index *core.Index) string {
