...
func (engine *Engine) logSQLQueryTime(sqlStr string, args []interface{}, executionBlock func() (*core.Stmt, *core.Rows, error)) (*core.Stmt, *core.Rows, error) {
	if engine.showSQL && engine.showExecTime {
		...
		if len(args) > 0 {
			engine.logger.Sqlf("[SQL] %s %v - took: %v", sqlStr, args, execDuration)
		} else {
			engine.logger.Sqlf("[SQL] %s - took: %v", sqlStr, execDuration)
		}
		return stmt, res, err
	}
...
func (engine *Engine) logSQLExecutionTime(sqlStr string, args []interface{}, executionBlock func() (sql.Result, error)) (sql.Result, error) {
	if engine.showSQL && engine.showExecTime {
		...
		execDuration := time.Since(b4ExecTime)
		if len(args) > 0 {
			engine.logger.Sqlf("[sql] %s [args] %v - took: %v", sqlStr, args, execDuration)
		} else {
			engine.logger.Sqlf("[sql] %s - took: %v", sqlStr, execDuration)
		}
		return res, err
...
func (engine *Engine) getFK(v reflect.Value) ([]core.Column, error) {
	t := v.Type()
	...
	table.Type = t
	var fkColumns []core.Column
	for i := 0; i < t.NumField(); i++ {
		tag := t.Field(i).Tag
		col := core.Column{Name: strings.ToLower(t.Field(i).Name), FieldName: strings.ToLower(t.Field(i).Name), Nullable: true, IsPrimaryKey: false,
			IsAutoIncrement: false, MapType: core.TWOSIDES, Indexes: make(map[string]int), TableName: table.Name}
		fkTagStr := tag.Get(engine.TagFK)
		if fkTagStr != "" {
			tags := splitTag(fkTagStr)
			col.ForeignKey = tags[0]
			...
			if len(tags) > 0 {
				...
				for j, key := range tags {
					...
					if pStart > -1 {
						...
						//var err error
						//if len(ctx.params)>1{
						//	l1,err=strconv.ParseInt(ctx.params[0],10,32)
						//	if err!=nil {
						//		l2, _ = strconv.ParseInt(ctx.params[1], 10, 32)
						//		col.Length = int(l1)
						//		col.Length2 =int(l2)
						//	}
						//}else{
						//	l,err=strconv.ParseInt(ctx.params[0],10,64)
						//	if err!=nil {
						//		col.Length =int(l)
						//		col.Length2 = 0
						//	}
...
WHERE  A.CONSTID = O3.ID AND A.FKEYID = O1.ID AND A.RKEYID = O2.ID AND L1.ID = O1.ID AND L2.ID = O2.ID AND A.FKEY = L1.COLID AND A
	...
	f_table_name := ""
	f_column_name := ""
	temp := strings.Split(column.ForeignKey, "(")
	f_table_name = temp[0]
		...
		if !isExist {
			...
			for _, col := range table.Columns() {
				...
				if !isExist {
					...
					if (col.XormTag != phyCol.XormTag) && !col.IsPrimaryKey {
...
func (engine *Engine) AlterAutoIncrement(beans ...interface{}) error {
	...
	for _, bean := range beans {
		err = session.AlterIncrement(bean)
		if err != nil {
			session.Rollback()
			return err
		}
