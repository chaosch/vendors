...
func genCols(table *core.Table, session *Session, bean interface{}, useCol bool, includeQuote bool, isUpdate bool) ([]string, []interface{}, error) {
	...
	for _, col := range table.Columns() {
		...
		if !session.IdentityInsert { //自增长字段为0时是否在列列表中添加自增长字段，默认值为false
		...
		if (isUpdate && col.IsUpdated) && session.Statement.UseAutoTime&&fieldValue.IsNil() /*&& isZero(fieldValue.Interface())*/ {
		...
		} else if (!isUpdate && col.IsCreated) && session.Statement.UseAutoTime&&fieldValue.IsNil() {
			val, t := session.Engine.NowTime2(col.SQLType.Name)
			args = append(args, val)
