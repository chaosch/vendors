	...
	if err != nil {
		...
		if verValue != nil && verValue.IsValid() && verValue.CanSet() {
			if verValue.Kind() == reflect.Ptr {
				var i int64
				if verValue.Elem().IsValid() {
					i = int64(verValue.Elem().Int() + 1)
				} else {
					i = 1
				}
...
func (session *Session) UpdateNull(bean interface{}, condiBean ...interface{}) (int64, error) {
	defer session.resetStatement()
	if session.IsAutoClose {
		defer session.Close()
	}
	...
	if isStruct {
		...
		if len(session.Statement.NullFields) > 0 {
			for _, cn := range session.Statement.NullFields {
				colNames = append(colNames, cn+"=null")
			}
		} else {
			return 0, errors.New("没有设置置空字段!")
		}
	} else {
		return 0, ErrParamsType
	}
...
func SliceRemove(slice []interface{}, index int) []interface{} {
	...
	for idx := range slice {
		if idx != index {
			temp = append(temp, slice[idx])
		}
	}
	return temp
}

func (session *Session) UpdateWhere(bean interface{}, WhereString string) (int64, error) {
	defer session.resetStatement()
	if session.IsAutoClose {
		defer session.Close()
	}
