...
type Session struct {
	...
	IsAutoClose            bool
	//SqlParseRes            *sqlparse.SQLParserResult
...
//func (session *Session) SetIdentityInsert(IdentyInsert bool, table *core.Table, resetAutoIncrease bool) {
//	tableName := table.Name
//	if IdentyInsert {
//		session.IdentityInsert = true
//		if session.Engine.Dialect().DBType() == core.MSSQL {
//			session.Exec("set IDENTITY_INSERT " + tableName + " ON")
//		}
//		if session.Engine.Dialect().DBType() == core.MYSQL {
//			session.Exec("SET SESSION sql_mode='NO_AUTO_VALUE_ON_ZERO';")
//		}
//
//	} else {
//		if session.Engine.Dialect().DBType() == core.ORACLE {
//			//// create sequence seq_2 start with -1 minvalue -1;
//			//if lastid > 0 {
//			session.Exec("drop sequence seq_" + tableName)
//			//	session.Exec("create  sequence seq_" + tableName + " start with " + strconv.FormatInt(maxid+1, 10) + " minvalue 0")
//			//}
//		}
//		if session.Engine.Dialect().DBType() == core.MYSQL {
//			//			session.Exec("SET SESSION sql_mode='NO_AUTO_VALUE_ON_ZERO';")
...
//			session.Exec("set IDENTITY_INSERT " + tableName + " OFF")
//		}
//		session.IdentityInsert = false
//		if table != nil && resetAutoIncrease {
//			if len(table.AutoIncrement) > 0 {
//				sqlMaxId := fmt.Sprintf("select max(%s)+1 maxid from %s", table.AutoIncrement, tableName)
//				res, err := session.QueryString(sqlMaxId)
//				if err == nil {
//					startWith := res[0]["maxid"]
//					iStartWith, err := strconv.ParseInt(startWith, 10, 64)
//					if err == nil {
//						if strings.HasPrefix(table.Name, "tab_") {
//							if iStartWith > core.Idt.Idprefix {
//								table.PKColumns()[0].StartWith = iStartWith
//								sql := session.Engine.Dialect().AlterIncrementSql(table, tableName, "", "")
//								session.Exec(sql)
//							}
//						} else {
//							table.PKColumns()[0].StartWith = iStartWith
