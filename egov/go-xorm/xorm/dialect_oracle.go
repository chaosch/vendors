...
func (db *oracle) GetTablesSingle(tableName string) ([]*core.Table, error) {
	args := []interface{}{}
	s := "SELECT lower(table_name) table_name FROM user_tables and lower(table_name)='" + tableName + "'"
	db.LogSQL(s, args)
...
func (db *oracle) CreateRenameIndexSql(tableName string, oldIndex, index *core.Index) string {
	sql := "alter index %s rename to %s"
	return fmt.Sprintf(sql, oldIndex.Name, index.Name)
}
