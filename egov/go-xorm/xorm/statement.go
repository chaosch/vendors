...
func buildConds(engine *Engine, table *core.Table, bean interface{},
	...
	if len(condParams) > 0 {
		if reflect.TypeOf(condParams[0]).Kind()!=reflect.Bool{
			return nil,errors.New("condParams[0] useOr need bool data type")
		}
		useOr = condParams[0].(bool)
	}
	if len(condParams) > 1 {
		if reflect.TypeOf(condParams[1]).Kind()!=reflect.Bool{
			return nil,errors.New("condParams[1] useLike need bool data type")
		}
...
//
func (statement *Statement) SetNullFields(str []string) *Statement {
	statement.NullFields = str
	return statement
}
...
func (statement *Statement) genCreateTableSQL() string {
	if statement.Engine.IdentityInsert {
		if statement.RefTable.AutoIncrement != "" {
			...
			statement.RefTable.AutoIncrement = ""
		}
	}
